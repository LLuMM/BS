...
def loadContent(item):
    ...
    if len(nodes.xpath('//div[@itemprop="articleBody"]'))> 0:
        ...
        cont = content.xpath("string(.)").encode("utf-8")
	contents['content'] = cont
	
	contents['auther'] = nodes.xpath('//div/a[@class="username ellipsis"]/text()')
	print contents['auther']
	contents['date'] = nodes.xpath('//div/time') 
	print contents['date']
...
def writePage(items):
    ...
    for item in items:
	    ...
	    nid = uuid.uuid1()
            n_id = uuid.uuid4()
	    sqlc = "insert into news(nid, title, n_id, type, date, source, url) values('%s', '%s', '%s', '%d', '%s', '%s', '%s')"%(nid, item['title'],n_id, 1, times, '掘金', item['href'])
