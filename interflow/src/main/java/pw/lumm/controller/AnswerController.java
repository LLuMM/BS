...
public class AnswerController extends BaseController{
    ...
    public void addAnswerToUser(HttpServletResponse out,String qid, String fromid, String toid,String content,int type) throws IOException {
        out.setContentType("text/html; charset=utf-8");
        try {
            ansewerServer.addAnswerToUser(qid, fromid,toid, content,type);
            response.Status = true;
        }catch (Exception e) {
            response.Status = false;
            response.Message = e.getMessage();
