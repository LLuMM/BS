package pw.lumm.utils.websocket;
...
import pw.lumm.utils.GsonUtils;
...
public class ChatWebSocketHandler implements WebSocketHandler {
	...
	public void afterConnectionEstablished(WebSocketSession webSocketSession) throws Exception {
		...
		USER_SOCKETSESSION_MAP.put(user.getUid(), webSocketSession);
		
		//群发消息告知大家
		Msg msg = new Msg();
		msg.setContent("【"+user.getUsername()+"】加入群聊！");
	...
	public void handleMessage(WebSocketSession webSocketSession, WebSocketMessage<?> message) throws Exception {
		...
		//反序列化服务端收到的json消息
		Msg msg = GsonUtils.fromJson(message.getPayload().toString(), Msg.class);
		msg.setTime(DateUtil.format(new Date()));
		//处理html的字符，转义：
	...
	public void handleTransportError(WebSocketSession webSocketSession, Throwable exception) throws Exception {
		...
		for (Entry<String, WebSocketSession> entry : entrySet) {
			if(entry.getKey().equals(user.getUid())){
				msg.setContent("【"+user.getUsername()+"】已经退出。。。！");
				//清除在线会话
	...
	public void afterConnectionClosed(WebSocketSession webSocketSession, CloseStatus closeStatus) throws Exception {
		...
		for (Entry<String, WebSocketSession> entry : entrySet) {
			if(entry.getKey().equals(user.getUid())){
				//群发消息告知大家
				msg.setContent("【"+user.getUsername()+"】已经有事先走了，大家继续聊...");
				//清除在线会话
				USER_SOCKETSESSION_MAP.remove(entry.getKey());
				//记录日志：
				System.out.println("Socket会话已经移除:用户ID" + entry.getKey());
